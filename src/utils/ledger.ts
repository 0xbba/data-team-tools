...
export function parseLedgerText(text: string): Omit<LedgerRecord, '_dbId' | '_deleted'> | null {
  ...
  // 找到第一个未完成的环节
  const isAcceptor = (name: string): boolean => name?.includes('数据需求承接') || name?.includes('数据承接') || name?.includes('数据统计人员')
  const isCoordinator = (name: string): boolean => name?.includes('数据需求统筹') || name?.includes('需求统筹')
